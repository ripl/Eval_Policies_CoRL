...
REPLACEMENT_ID="${REPLACEMENT_ID:-dexbotic_replacement_$(date -u +%Y%m%dT%H%M%SZ)}"
...
declare -A failed_jobs=(
  [protocol_A]=2075427
  [protocol_B]=2075428
  [protocol_C1_yellow_on_green]=2075429
  [protocol_C2_blue_on_red]=2075430
  [protocol_C3_red_on_blue]=2075431
  [protocol_D]=2075432
  [protocol_E]=2075433
)
...
usage() {
  cat <<'EOF'
Usage: submit_dexbotic_replacements.sh [--dry-run] [--preflight-only] [--allow-submit]

Submits exactly seven Dexbotic replacement jobs for the existing Protocol A-E
full run. No Slurm arrays are used.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --replacement-id) REPLACEMENT_ID="$2"; REPLACEMENT_ROOT="${RUN_ROOT}/dexbotic_replacements/${REPLACEMENT_ID}"; LOG_DIR="${RUN_ROOT}/logs/slurm/${REPLACEMENT_ID}"; MANIFEST="${RUN_ROOT}/bundle/${REPLACEMENT_ID}_submission.tsv"; PREFLIGHT_REPORT="${RUN_ROOT}/bundle/${REPLACEMENT_ID}_preflight_report.json"; shift 2 ;;
    -h|--help) usage; exit 0 ;;
...
job_name_for() {
  printf 'se_abcde_dexbotic_%s_r1' "$(condition_short "$1")"
}

failed_result_dir_for() {
  printf '%s/dexbotic/%s' "${RUN_ROOT}" "$1"
}

replacement_result_dir_for() {
...
failed_stdout_for() {
  ...
  printf '%s/logs/slurm/se_abcde_dexbotic_%s-%s.out' "${RUN_ROOT}" "${short}" "${failed_jobs[${condition}]}"
}
...
failed_stderr_for() {
  ...
  local short
  short="$(condition_short "${condition}")"
  printf '%s/logs/slurm/se_abcde_dexbotic_%s-%s.err' "${RUN_ROOT}" "${short}" "${failed_jobs[${condition}]}"
}

export_vars_for() {
  local condition="$1"
  local output_root="$2"
  printf 'ALL,PROJECT_ROOT=%s,SUPPORT_ROOT=%s,RUN_ID=%s,RUN_ROOT=%s,POLICY=dexbotic,CONDITION=%s,OUTPUT_ROOT=%s,ASSET_MANIFEST=%s,SIMPLERENV_PROTOCOL_CONDITION=%s,SIMPLERENV_PROTOCOL_CONFIG=%s,SIMPLERENV_PROTOCOL_SHA256=%s,DEXBOTIC_NORM_STATS_SOURCE=%s,DEXBOTIC_NORM_STATS_SHA256=%s,PYTHONDONTWRITEBYTECODE=1' \
    "${PROJECT_ROOT}" \
    "${SUPPORT_ROOT}" \
    "${SOURCE_RUN_ID}" \
    "${RUN_ROOT}" \
...
payload = {
    ...
    "horizon": 60,
    "protocol_sha256": "1f2b4ea48e38df7d25304638d998c40902d79634433f725f0895e26f04ad810b",
    "fix": "server launcher is bundled and bind-mounted; strict norm_stats.json is copied into /dex_run with SHA verification",
    "support_files": {
...
check_static() {
  ...
  bash -n "${SUPPORT_ROOT}/submit_dexbotic_replacements.sh"
  PYTHONPATH="${SUPPORT_ROOT}:${SUPPORT_ROOT}/bin:${PYTHONPATH:-}" python3 -m py_compile \
...
from protocol_abcde_common import CONDITIONS, EPISODE_IDS, HORIZON, load_protocol_config
load_protocol_config()
assert len(CONDITIONS) == 7, CONDITIONS
...
PY
  ...
  for condition in "${conditions[@]}"; do
    require_file "$(failed_stdout_for "${condition}")"
    require_file "$(failed_stderr_for "${condition}")"
    if [[ ! -d "$(failed_result_dir_for "${condition}")" ]]; then
      echo "missing failed result dir: $(failed_result_dir_for "${condition}")" >&2
      exit 2
    fi
    if [[ -e "$(replacement_result_dir_for "${condition}")" ]]; then
      echo "refusing to reuse replacement result dir: $(replacement_result_dir_for "${condition}")" >&2
      exit 2
    fi
  done
  write_preflight_report
...
dry_run_commands() {
  ...
  for condition in "${conditions[@]}"; do
    ...
    output_root="$(replacement_result_dir_for "${condition}")"
  ...
  if [[ "${count}" != "7" ]]; then
    echo "ERROR: dry-run command count ${count}, expected 7" >&2
    exit 1
  fi
  echo "dry_run_count=7"
...
if [[ -e "${MANIFEST}" ]]; then
  echo "refusing to overwrite replacement manifest: ${MANIFEST}" >&2
  exit 2
fi
mkdir -p "${LOG_DIR}" "$(dirname "${MANIFEST}")" "${REPLACEMENT_ROOT}"
tmp="${MANIFEST}.tmp"
printf 'replacement_job_id\tfailed_job_id\tpolicy\tcondition\tjob_name\tfailed_result_dir\treplacement_result_dir\tfailed_stdout_log\tfailed_stderr_log\tsbatch_script\tstdout_log\tstderr_log\tprotocol_sha256\tsubmitted_at_utc\tsource_run_id\treplacement_id\tsupport_root\tfix_summary\n' > "${tmp}"
count=0
for condition in "${conditions[@]}"; do
  job_name="$(job_name_for "${condition}")"
  job_id="$(submit_one "${condition}")"
  job_num="${job_id%%;*}"
  printf '%s\t%s\tdexbotic\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "${job_id}" \
    "${failed_jobs[${condition}]}" \
    "${condition}" \
    "${job_name}" \
    "$(failed_result_dir_for "${condition}")" \
    "$(replacement_result_dir_for "${condition}")" \
    "$(failed_stdout_for "${condition}")" \
    "$(failed_stderr_for "${condition}")" \
    "${SUPPORT_ROOT}/run_dexbotic_condition.sbatch" \
    ...
    "${REPLACEMENT_ID}" \
    "${SUPPORT_ROOT}" \
    "bundled Dexbotic seeded server launcher plus strict norm_stats bind into /dex_run" >> "${tmp}"
  count=$((count + 1))
done
if [[ "${count}" != "7" ]]; then
  echo "ERROR: submitted ${count} jobs, expected 7" >&2
  exit 1
fi
mv "${tmp}" "${MANIFEST}"
echo "replacement_manifest=${MANIFEST}"
