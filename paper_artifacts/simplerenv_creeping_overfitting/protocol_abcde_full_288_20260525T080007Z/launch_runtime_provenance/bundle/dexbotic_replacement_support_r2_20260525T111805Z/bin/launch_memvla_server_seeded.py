...
def main() -> None:
    ...
    print(
        "Starting seeded Dexbotic Simpler MemVLA server "
        f"seed={seed} model={exp.inference_config.model_name_or_path} "
        f"port={exp.inference_config.port} norm_stats={exp.inference_config.norm_stats}",
        flush=True,
    )
    exp.inference()
