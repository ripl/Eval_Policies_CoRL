...
RUN_TAG="${RUN_TAG:-protocol2_spatialvla_$(date -u +%Y%m%dT%H%M%SZ)}"
...
RESULTS_DIR="${RESULTS_DIR:-${PROJECT_ROOT}/results/simplerenv/spatialvla_bridge/protocol2_random_positions_robot_initial_states/${RUN_TAG}}"
...
run_task() {
  ...
  echo "=== SpatialVLA Protocol 2 ${label}: ${env_name} episodes ${EPISODE_START}:${EPISODE_END} ==="
