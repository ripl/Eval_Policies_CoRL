...
RUN_TAG="${RUN_TAG:-protocol2_xvla_${TASK_LABEL:-spoon}_$(date -u +%Y%m%dT%H%M%SZ)}"
...
RESULTS_DIR="${RESULTS_DIR:-${PROJECT_ROOT}/results/simplerenv/xvla_widowx/protocol2_random_positions_robot_initial_states/${RUN_TAG}}"
...
case "${TASK_LABEL}" in
  stack|blocks|carrot|spoon)
    ...
    ;;
  *)
    echo "Protocol 2 supports TASK_LABEL=stack|blocks|carrot|spoon, got ${TASK_LABEL}" >&2
    exit 2
    ;;
esac
