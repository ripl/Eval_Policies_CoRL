...
RUN_TAG="${RUN_TAG:-protocol1_cogact_$(date -u +%Y%m%dT%H%M%SZ)}"
...
RESULTS_DIR="${RESULTS_DIR:-${PROJECT_ROOT}/results/simplerenv/cogact_base/protocol1_random_positions/${RUN_TAG}}"
...

{
  ...
  echo "policy=cogact"
  echo "protocol=widowx_protocol1_random_positions"
...
run_task() {
  ...
  echo "=== CogACT Protocol 1 ${label}: ${env_name} episodes ${EPISODE_START}:${EPISODE_END} ==="
  python "${PROTOCOL_MAIN}" \
    ...
    --additional-env-save-tags protocol1_seed20260515 \
...
summary = {
    "policy": "cogact",
    "protocol": "widowx_protocol1_random_positions",
