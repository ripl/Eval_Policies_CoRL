...
RUN_TAG="${RUN_TAG:-xvla_widowx_spoon_$(date -u +%Y%m%dT%H%M%SZ)}"
ENV_PREFIX="${ENV_PREFIX:-${PROJECT_ROOT}/envs/simplerenv_xvla_py310}"
MODEL_PATH="${MODEL_PATH:-2toINF/X-VLA-WidowX}"
TASK_CLIENT="${TASK_CLIENT:-client_spoon.py}"
TASK_LABEL="${TASK_LABEL:-spoon}"
RESULTS_DIR="${RESULTS_DIR:-${PROJECT_ROOT}/results/simplerenv/xvla_widowx/official_fixed_grid/${RUN_TAG}}"
PORT="${PORT:-$((18000 + (${SLURM_JOB_ID:-0} % 10000)))}"
case "${TASK_LABEL}" in
  spoon|carrot|stack|blocks)
    TASK_MAX_STEPS="${TASK_MAX_STEPS:-60}"
    ;;
  eggplant)
    TASK_MAX_STEPS="${TASK_MAX_STEPS:-120}"
    ;;
  *)
    TASK_MAX_STEPS="${TASK_MAX_STEPS:-60}"
    ;;
...
mkdir -p "${RESULTS_DIR}" "${SERVER_DIR}" "${CLIENT_DIR}" "${METADATA_DIR}"
...
export PYTHONPATH="${X_VLA_REPO}:${SIMPLER_ENV_REPO}:${PYTHONPATH:-}"
...
cd "${X_VLA_REPO}/evaluation/simpler/WidowX"
python "${TASK_CLIENT}" --server_ip 127.0.0.1 --server_port "${PORT}" --output_dir "${CLIENT_DIR}"
...
if len(rows) != 24 or errors or missing_done:
    raise SystemExit(f"invalid X-VLA results: rows={len(rows)} errors={len(errors)} missing_done={len(missing_done)}")
standard_horizon = int("${TASK_MAX_STEPS}")
released_success = sum(1 for r in rows if bool(r["done"]))
standard_success = sum(1 for r in rows if bool(r["done"]) and int(r.get("steps", 10**9)) <= standard_horizon)
late_successes = [
...
summary = {
    ...
    "success_released_client": released_success,
    "success_rate_released_client": released_success / len(rows),
    "total": len(rows),
