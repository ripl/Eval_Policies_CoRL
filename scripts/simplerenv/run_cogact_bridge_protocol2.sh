...
RUN_TAG="${RUN_TAG:-protocol2_cogact_$(date -u +%Y%m%dT%H%M%SZ)}"
...
RESULTS_DIR="${RESULTS_DIR:-${PROJECT_ROOT}/results/simplerenv/cogact_base/protocol2_random_positions_robot_initial_states/${RUN_TAG}}"
PROTOCOL_CONFIG="${PROTOCOL_CONFIG:-${PROJECT_ROOT}/configs/simplerenv/randomized_positions/widowx_protocol2_seed20260515_n500.json}"
...
case "${TASK_FILTER}" in
  ...
  *) echo "Protocol 2 supports TASK_FILTER=all|stack|carrot|spoon, got ${TASK_FILTER}" >&2; exit 2 ;;
esac
...
export SIMPLERENV_TARGET_MAIN="${COGACT_MAIN}"
...
export PYTHONPATH="${PROJECT_ROOT}/scripts/simplerenv:${COGACT_REPO}:${SIMPLER_ENV_REPO}:${PYTHONPATH:-}"

{
  ...
  echo "policy=cogact"
  echo "protocol=widowx_protocol2_random_positions_robot_initial_states"
...
run_task() {
  ...
  echo "=== CogACT Protocol 2 ${label}: ${env_name} episodes ${EPISODE_START}:${EPISODE_END} ==="
  python "${PROTOCOL_MAIN}" \
    ...
    --rgb-overlay-path "${SIMPLER_OVERLAY_DIR}/bridge_real_eval_1.png" \
    ...
    --additional-env-save-tags protocol2_seed20260515 \
...
summary = {
    "policy": "cogact",
    "protocol": "widowx_protocol2_random_positions_robot_initial_states",
